  ...
  try {
    ...
    if (data.type === "expense" && data.categoryId) {
      ...
      if (anyBudget) {
        ...
        if (!activeBudget) {
          return { success: false, error: "No active budget found for this period. Please make a new budget before adding a transaction." };
        }
      }
    }
    ...
    await logAuditEvent("Transaction", (transaction as any)._id.toString(), "CREATE", undefined, transaction);
    ...
    if (status === "completed") {
      if (data.paymentMode === "credit_card" && data.creditCardId) {
        ...
        if (card) {
          ...
          if (!statement) {
            ...
            statement = await CardStatement.create({
              ...
              minimumDue: (finalAmount * card.minimumDuePercent) / 100,
              paymentStatus: "unpaid",
              transactions: [(transaction as any)._id],
            });
          } else {
            statement.totalAmount += finalAmount;
            statement.minimumDue = (statement.totalAmount * card.minimumDuePercent) / 100;
            statement.transactions.push((transaction as any)._id as any);
        ...
        if (destAcc) {
          const addAmt = data.destinationAmount !== undefined ? data.destinationAmount : (destAcc.currency === currency ? originalAmount : getConversionRate(destAcc.currency, await fetchExchangeRates()) * finalAmount);
      ...
      if (data.loanId) {
        ...
        const loan = await Loan.findOne({ _id: data.loanId, userId: session.user.id });
        if (loan) {
...
export async function deleteTransaction(id: string) {
  try {
    ...
    if (!transaction) return { success: false, error: "We couldn't find the requested transaction. It may have been deleted." };
...
export async function confirmTransaction(id: string, status: "completed" | "cancelled" | "pending") {
  try {
    ...
    if (transaction.status === status) return { success: true, data: JSON.parse(JSON.stringify(transaction)) };
...
export async function updateTransaction(
  ...
  try {
    ...
    if (!oldTxn) return { success: false, error: "We couldn't find the requested transaction. It may have been deleted." };
      ...
      if (oldTxn.paymentMode === "credit_card" && oldTxn.creditCardId) {
        ...
        if (card) {
          ...
          if (statement) {
            ...
            statement.minimumDue = (statement.totalAmount * card.minimumDuePercent) / 100;
            statement.transactions = statement.transactions.filter((tId: any) => tId.toString() !== oldTxn._id.toString());
            await statement.save();
          }
        }
    ...
    // 2. Parse currency, final amount, exchange rate
    const originalAmount = Number(data.amount);
    let currency = data.originalCurrency;
    let account;
    if (data.accountId) {
    ...
    oldTxn.toAccountId = data.toAccountId as any;
    oldTxn.paymentMode = data.paymentMode;
    oldTxn.creditCardId = data.creditCardId as any;
    oldTxn.categoryId = data.categoryId as any;
    oldTxn.personId = data.personId as any;
    oldTxn.note = data.note;
    if (data.status) oldTxn.status = data.status;
    if (data.upiRef) oldTxn.upiRef = data.upiRef;
    if (data.upiPayeeName) oldTxn.upiPayeeName = data.upiPayeeName;
    if (data.upiPayeeVpa) oldTxn.upiPayeeVpa = data.upiPayeeVpa;

    await oldTxn.save();

    // 4. Apply the impact of the new transaction if it is completed
    const newStatus = oldTxn.status || "completed";
    if (newStatus === "completed") {
      if (data.paymentMode === "credit_card" && data.creditCardId) {
        ...
        if (card) {
          ...
          if (!statement) {
            ...
            statement = await CardStatement.create({
              ...
              minimumDue: (finalAmount * card.minimumDuePercent) / 100,
              paymentStatus: "unpaid",
              transactions: [oldTxn._id],
            });
          } else {
            statement.totalAmount += finalAmount;
            statement.minimumDue = (statement.totalAmount * card.minimumDuePercent) / 100;
            if (!statement.transactions.includes(oldTxn._id as any)) {
        ...
        const destAcc = await Account.findOne({ _id: data.toAccountId, userId: session.user.id });
        if (destAcc) {
          const addAmt = destAcc.currency === currency ? originalAmount : getConversionRate(destAcc.currency, await fetchExchangeRates()) * finalAmount;
          destAcc.balance += addAmt;
          await destAcc.save();

          oldTxn.destinationAmount = addAmt;
    ...
    return { success: true, data: JSON.parse(JSON.stringify(oldTxn)) };
  } catch (err: any) {
