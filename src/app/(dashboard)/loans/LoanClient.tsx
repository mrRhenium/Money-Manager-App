...
export function LoanClient({ loans, accounts }: { loans: any[], accounts: any[] }) {
  ...
  const totalAssets = loans.filter(l => l.status === "active" && l.type === "given").reduce((sum, l) => sum + l.outstandingBalance, 0);
  ...
  const activeLoans = useMemo(() => {
    const filtered = loans.filter(l => l.status === "active" && l.name.toLowerCase().includes(searchTerm.toLowerCase()));
    return sortLoans(filtered);
  }, [loans, searchTerm, sortBy]);

  const completedLoans = useMemo(() => {
    const filtered = loans.filter(l => l.status === "completed" && l.name.toLowerCase().includes(searchTerm.toLowerCase()));
    return sortLoans(filtered);
  }, [loans, searchTerm, sortBy]);
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4 bg-card p-3 rounded-xl border shadow-sm items-center">
          ...
          <div className="relative flex-1 w-full">
            <AntSelect
              ...
              { label: "💰 Progress: Most Paid First", value: "prog-high" },
          ...
          items={[
            {
              ...
              children: renderGrid(activeLoans),
            },
            {
              key: "completed",
              label: `Completed (${completedLoans.length})`,
              children: renderGrid(completedLoans),
            },
          ]}
        />
      </div>
