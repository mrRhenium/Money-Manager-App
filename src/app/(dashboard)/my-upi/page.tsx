      ...
      <div className="shrink-0 h-20 z-40 dark:text-white bg-background/95 dark:bg-card/95 backdrop-blur-2xl shadow-[1px_0_40px_rgba(0,0,0,0.02)] dark:shadow-[1px_0_40px_rgba(0,0,0,0.1)] border-b py-4 px-4 lg:px-8">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-foreground flex items-center gap-2">
                <QrCode className="w-6 h-6 text-primary" />My UPI & QR
              </h1>
          ...
          <div className="flex items-center gap-3">
            <Button onClick={handleSave} disabled={isSaving} className="shadow-sm px-6">
              {isSaving ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Saving...</> : "💾 Save UPI IDs"}
            </Button>
          </div>
        </div>
      </div>
          ...
          <div className="grid gap-2 sm:gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
            ...
            <Card className="shadow-sm bg-gradient-to-br from-card to-card hover:to-primary/5 dark:hover:to-primary/10 border border-slate-200/60 dark:border-slate-800 transition-all duration-300 relative overflow-hidden">
              ...
              <CardHeader className="flex flex-row items-center justify-between space-y-0 p-4 pb-1">
                <CardTitle className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Total UPI IDs</CardTitle>
              </CardHeader>
              <CardContent className="p-4 pt-1">
                <p className="text-2xl font-bold text-foreground">{activeIds.length}</p>
                <p className="text-xs text-muted-foreground mt-1">Registered IDs</p>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-sm bg-gradient-to-br from-card to-card hover:to-emerald-500/5 dark:hover:to-emerald-500/10 border border-slate-200/60 dark:border-slate-800 transition-all duration-300 relative overflow-hidden">
              ...
              <CardHeader className="flex flex-row items-center justify-between space-y-0 p-4 pb-1">
                <CardTitle className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Primary UPI</CardTitle>
              </CardHeader>
              <CardContent className="p-4 pt-1">
                <p className="text-lg font-bold text-foreground font-mono truncate">{activeIds[0] || "Not set"}</p>
                <p className="text-xs text-muted-foreground mt-1">Default receiving ID</p>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-sm bg-gradient-to-br from-card to-card hover:to-purple-500/5 dark:hover:to-purple-500/10 border border-slate-200/60 dark:border-slate-800 transition-all duration-300 relative overflow-hidden">
              <div className="absolute top-0 right-0 p-3 opacity-50 text-purple-500">
                <Shield className="w-5 h-5" />
              </div>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 p-4 pb-1">
                <CardTitle className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">QR Status</CardTitle>
              </CardHeader>
              <CardContent className="p-4 pt-1">
                <p className={`text-lg font-bold ${activeIds.length > 0 ? 'text-emerald-500' : 'text-amber-500'}`}>
                  {activeIds.length > 0 ? "✅ Active" : "⚠️ Inactive"}
                </p>
                <p className="text-xs text-muted-foreground mt-1">{activeIds.length > 0 ? "QR code ready" : "Add a UPI ID first"}</p>
              </CardContent>
            </Card>
          </div>
            ...
            <Card className="lg:col-span-3 shadow-sm border border-slate-200/60 dark:border-slate-800">
              <CardHeader className="border-b border-border/50">
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-lg font-bold text-foreground">My UPI IDs</CardTitle>
                    <p className="text-sm text-muted-foreground mt-1">Add your VPA/UPI IDs to generate receiving QR codes.</p>
                  </div>
