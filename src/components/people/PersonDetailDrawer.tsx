...
export function PersonDetailDrawer({
  ...
  // Accurate remaining net balance
  const remainingBalance = totalGiven - totalReceived;
          ...
          <div className="mt-3 p-4 rounded-xl border bg-card shadow-sm flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${
                ...
                {remainingBalance > 0 ? <ArrowDownLeft className="w-5 h-5" /> : remainingBalance < 0 ? <ArrowUpRight className="w-5 h-5" /> : <CheckCircle2 className="w-5 h-5" />}
              </div>
              <div>
                <p className="text-xs text-muted-foreground font-medium">Remaining Net Position</p>
                <p className={`text-base font-extrabold ${
                  ...
                  {remainingBalance > 0 
                    ? `You will receive ${format(remainingBalance)}` 
                    : remainingBalance < 0 
                      ? `You owe / To Pay ${format(Math.abs(remainingBalance))}` 
                      : 'All Settled (₹0)'}
                </p>
                {totalGiven > 0 && totalReceived > 0 && remainingBalance !== 0 && (
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    {format(totalReceived)} received of {format(totalGiven)} ({format(Math.abs(remainingBalance))} remaining)
                  </p>
                )}
              </div>
            </div>
            ...
            <span className={`px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider border shrink-0 ${
              ...
              {remainingBalance > 0 ? "They Owe" : remainingBalance < 0 ? "You Owe" : "Settled"}
            </span>
          </div>
        </div>
