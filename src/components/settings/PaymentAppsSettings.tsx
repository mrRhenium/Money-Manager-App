...
export function PaymentAppsSettings({ noBorder = false }: PaymentAppsSettingsProps) {
  ...
  return (
    <div className={`space-y-6 ${noBorder ? "" : "p-0"}`}>
      {/* Information Banner */}
      <div className="p-4 rounded-xl bg-primary/5 border border-primary/10 flex items-start gap-3">
        ...
        <div className="text-xs text-muted-foreground leading-relaxed">
          <p className="font-semibold text-foreground text-sm mb-0.5">UPI & Payment Apps Manager</p>
          Mark which payment apps you use on your phone (Google Pay, PhonePe, Paytm, Amazon Pay, etc.). Active apps will appear at the top in <span className="font-semibold text-foreground">Scan & Pay</span> with green <span className="text-emerald-600 font-semibold">Active</span> badges.
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-3 min-w-0">
                <div 
                  className="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm shrink-0 shadow-2xs border"
                  style={{
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <h4 className="text-sm font-bold text-foreground truncate">{app.name}</h4>
                    {isDef && (
                      <span className="px-1.5 py-0.5 rounded text-[10px] font-extrabold bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 flex items-center gap-0.5 shrink-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className={`text-[11px] font-semibold flex items-center gap-1 ${
                      ...
                      {app.isActive ? "Active (In Use)" : "Not in use"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                >
                  <Star className={`w-4 h-4 ${isDef ? "fill-amber-500 text-amber-500" : ""}`} />
                </button>
