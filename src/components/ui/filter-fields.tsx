...
const commonClasses = "bg-background text-foreground border-input placeholder:text-muted-foreground focus-visible:ring-1 focus-visible:ring-primary/50 shadow-sm transition-colors";
...
export function FilterInput({ label, className, ...props }: FilterInputProps) {
  return (
    <div className="space-y-1.5 w-full">
      ...
      <Input 
        className={cn("h-9", commonClasses, className)} 
        {...props} 
      />
    </div>
  );
  ...
  return (
    <div className="space-y-1.5 w-full">
      ...
      <Select value={value} onValueChange={(val: any) => onValueChange?.(val)}>
        <SelectTrigger className={cn("h-9", commonClasses, className)}>
